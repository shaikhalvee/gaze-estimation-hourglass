...
import numpy as np
import cv2
from gazeNeuralNetwork.utils.neural_functions import gaussian_2d
from matplotlib import pyplot as plt
from gazeNeuralNetwork.utils.gaze_functions import draw_gaze

device = torch.device('cpu')
dataset = UnityEyesDataset()
checkpoint = torch.load('checkpoint.pt', map_location=device)
num_stack = checkpoint['nstack']
num_features = checkpoint['nfeatures']
num_landmarks = checkpoint['nlandmarks']
gaze_net = GazeNetwork(num_stack_of_hourglass=num_stack,
                       num_features=num_features,
                       num_landmarks=num_landmarks).to(device)
gaze_net.load_state_dict(checkpoint['model_state_dict'])

with torch.no_grad():
    sample = dataset[2]
    x = torch.tensor([sample['img']]).float().to(device)
    heatmaps = sample['heatmaps']
    heatmaps_pred, landmarks_pred, gaze_pred = gaze_net.forward(x)
