...
import torch
from datasets.unityeyes import UnityEyesDataset
from torch.utils.data import DataLoader
from gazeNeuralNetwork.neuralmodules import GazeNetwork
...
def validate(gaze_net: GazeNetwork, val_loader: DataLoader) -> float:
    with torch.no_grad():
        val_losses = []
        for val_batch in val_loader:
            val_images = val_batch['img'].float().to(device)
            heatmaps = val_batch['heatmaps'].to(device)
            landmarks = val_batch['landmarks'].to(device)
            gaze = val_batch['gaze'].float().to(device)
            heatmaps_pred, landmarks_pred, gaze_pred = gaze_net.forward(val_images)
            heatmaps_loss, landmarks_loss, gaze_loss = gaze_net.calc_loss(
...
def train_epoch(epoch: int,
    ...
    for i_batch, sample_batched in enumerate(train_loader):
        i_batch += N * epoch
        images = sample_batched['img'].float().to(device)
        heatmaps_pred, landmarks_pred, gaze_pred = gaze_net.forward(images)
        ...
        if i_batch > 0 and i_batch % 20 == 0:
            val_loss = validate(gaze_net=gaze_net, val_loader=val_loader)
            writer.add_scalar("validation loss", val_loss, i_batch)
            print('Epoch', epoch, 'Validation loss', val_loss)
            if val_loss < best_val_loss:
                best_val_loss = val_loss
                torch.save({
                    'nstack': gaze_net.num_stacks,
                    'nfeatures': gaze_net.num_features,
                    'nlandmarks': gaze_net.num_landmarks,
                    'best_val_loss': best_val_loss,
                    'model_state_dict': gaze_net.state_dict(),
...
def train(gaze_net: GazeNetwork, optimizer, nepochs: int, best_val_loss: float, checkpoint_fn: str):
...
def main():
    ...
    if args.start_from:
        ...
        gaze_net = GazeNetwork(num_stack_of_hourglass=nstack,
                               num_features=nfeatures,
                               num_landmarks=nlandmarks).to(device)
        optimizer = torch.optim.Adam(gaze_net.parameters(), lr=learning_rate)
        gaze_net.load_state_dict(start_from['model_state_dict'])
